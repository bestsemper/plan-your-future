...
function courseCountsForHSS(courseCode: string): boolean {
...
export function isHSSCourse(courseCode: string): boolean {
  return courseCountsForHSS(courseCode);
}
...
function getSatisfiedCredits(
  ...
  for (const takenCode of taken) {
    ...
    if (normalizedSubjectFilter) {
      // Special handling for HSS requirements
      if (normalizedSubjectFilter === 'HSS') {
        if (!courseCountsForHSS(normalizedCode)) {
          continue;
        }
      } else {
